...
- Never edit B1PRO/QC-PASS, an archived UF2, or an approved candidate in place.
...
| B1PRO/DESIGN/B1Pro-ZMK-KEYMAP-V2.md | Human-reviewed design illustrations. | Read only. |
| B1PRO/QC-IN_PROGRESS | Candidates, release records, checklists, unpassed UF2s. | Store the lab candidate here. |
| B1PRO/QC-PASS | Hardware-approved releases only. | Never edit. |
...
A compile proves syntax and link correctness only. Do not flash this UF2 without explicit instructor authorization. A real test-unit flash requires a revision entry in CHANGELOG.md, a release record, and a cumulative QC checklist. QC-PASS requires an explicit full hardware PASS.
...
5. What evidence is still needed before a built UF2 can enter B1PRO/QC-PASS?
