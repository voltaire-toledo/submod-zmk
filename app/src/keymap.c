...
int zmk_keymap_apply_position_state(uint8_t source, int layer, uint32_t position, bool pressed,
    ...
    LOG_DBG("layer: %d position: %d, binding name: %s,p1:%x,p2:%x", layer, position,
...
static inline uint8_t get_fn_exchange_state(void) { return fn_exchange; }
...
bool keyboard_os_is_mac(void) {
    return ((_zmk_keymap_layer_state & BIT(MAC_LAYER)) ||
            (_zmk_keymap_layer_state & BIT(MAC_LAYER + 1)) || _zmk_keymap_layer_state == MAC_LAYER);
}
...
void f1_f13_fn_exchange_start_check(void) {
    ...
    if (fn_exchange_flag_mac)
        f1_f13_fn_exchange_mac();
    if (fn_exchange_flag_win)
