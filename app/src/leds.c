...
static const struct led_effect led_peer_state_effect[] = {
    ...
    [LED_PEER_STATE_RECONN] = LED_EFFECT_LED_FLASH(
        3, 200, LED_COLOR(150, 0, 0)), // LED_EFFECT_LED_BLINK(200, LED_COLOR(150, 0, 0)),
...
static void timer_handler(struct k_timer *timer) {
    ...
    // LOG_INF("led
    // ptr:%x,time:%d,%d",led,led->effect->steps[0].substep_time,led->effect->steps[1].substep_time);
...
uint8_t keyboard_get_led_state(void) { return keyboard_led_state; }
uint8_t get_charge_led_state(void) { return charge_led_state; }
void blue_led_set_state(uint8_t led_state) {
    if (get_current_transport() != ZMK_TRANSPORT_BLE)
        return;
    LOG_DBG("set:%d", led_state);

    if ((leds[LED_BLUE].effect != &led_peer_state_effect[led_state]) ||
        k_timer_remaining_get(&leds[LED_BLUE].timer) ==
            0) // k_work_delayable_remaining_get(&leds[LED_BLUE].work)==0)
    {
...
void led_24G_set_state(uint8_t led_state) {
    ...
    if ((leds[LED_24G].effect != &led_peer_state_effect[led_state]) ||
        k_timer_remaining_get(&leds[LED_24G].timer) ==
            0) // k_work_delayable_remaining_get(&leds[LED_24G].work)==0)
    {
        LOG_DBG(".");
